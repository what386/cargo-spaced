...
pub fn edits_for_boundaries(source: &str, boundaries: &[Boundary]) -> Vec<Edit> {
    boundaries
        .iter()
        .filter_map(|boundary| edit_for_boundary(source, boundary))
        .collect()
}
...
fn rule_block_bodied_item_boundary(boundary: &Boundary) -> usize {
    if boundary.parent_kind == ContainerKind::ItemList
...
fn rule_multiline_let(boundary: &Boundary) -> usize {
    if boundary.parent_kind == ContainerKind::StatementList
